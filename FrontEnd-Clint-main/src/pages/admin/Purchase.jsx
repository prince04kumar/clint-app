...
export default function Purchase() {
    ...
    const fetchSales = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_APP_URL}/api/purchase/getall`);
            if (!response.ok) {
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                // setSold((sold)=>{return sold+parseInt(element.s)});
                // setPurchase((purchase)=>{return purchase+parseInt(element.z)});
                // console.log(element.s)
                sale_total+=element.s;
                // console.log(element.z)
                purchase_total+=element.z;
            }
            setSold(sale_total);
            setPurchase(purchase_total);
            // setLoading(false);
        } catch (err) {
            // setError(err);
            // setLoading(false);
    ...
    const deleteRemark = async (id) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_APP_URL}/api/purchase/del/${id}`, {
                method: 'DELETE'
            });
            if (!response.ok) {
                throw new Error(`Error: ${response.statusText}`);
    ...
    const updateSales = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_APP_URL}/api/purchase/update/${eleId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ z, s })
            });
            if (!response.ok) {
                throw new Error(`Error: ${response.statusText}`);
            }
    ...
    const addSales = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_APP_URL}/api/purchase/add`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ s, z })
            });
            if (!response.ok) {
                throw new Error(`Error: ${response.statusText}`);
            }
        ...
        <div className="bg-gray-100 font-family-karla flex sm:flex-row flex-col">
            ...
            <div className="w-full overflow-x-hidden border-t flex flex-col h-[100vh]">
                ...
                <main className="w-full flex-grow p-6">
                    <h1 className="text-3xl text-black pb-4">Purchase</h1>
                    ...
                    <div className="w-full mt-12 overflow-y-scroll">
                        ...
                        <div className="bg-white overflow-auto my-4 p-3 rounded-md shadow-sm border border-l-red-600 border-x-4 flex justify-between font-bold">
                            <div>SOLD : {sold}</div>
                            <div>PURCHASED : {purchase}</div>
                            <div>NET GAIN : {sold-purchase}</div><br />
                        </div>
                        {sales && sales.map((data, index) => {
                            return <div className="bg-white overflow-auto my-4 p-3 rounded-md shadow-sm border border-l-lime-600 border-x-4" id={data.salesId} key={index}>
                                ...
                                <div>SOLD : {data.s}</div>
                                <div>PURCHASED : {data.z}</div>
                                <div>NET GAIN : {data.s - data.z}</div><br />
                ...
                <div className='p-4 h-full flex justify-center items-center flex-col'>
                    ...
                    <form className="max-w-md mx-auto rounded border border-black p-5 container overflow-x-scroll">
                        ...
                        <div className="relative z-0 w-full mb-5 group">
                            ...
                            <label htmlFor="floating_pending" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">PURCHASED</label>
                        </div>
                        <div className="relative z-0 w-full mb-5 group">
                            ...
                            <label htmlFor="floating_adv" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">SOLD</label>
                        </div>
